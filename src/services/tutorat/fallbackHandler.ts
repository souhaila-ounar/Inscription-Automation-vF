...
export async function checkAndFallbackToOnline(data: {
  ...
  jobPayload.extra_attrs.note = `Manda créé automatiquement le ${dateFr} car le client attend 
  depuis 7 jours d'avoir un tuteur en présentiel et a accepté de passez en ligne.\n${notes}`;
  ...
  if (agentName) {
    ...
    const agentProjects = await supabase.getByFilter("pairing_agents", {
      slug: formattedAgent,
    });

    console.log(
      "formattedAgent : " + formattedAgent + "agentProjects : ",
      agentProjects
    );
    ...
    if (agentProjects.length > 0) {
      ...
      const taskResult = await asana.getTaskByName(
        "1131727432690028",
        projectId,
        job.name
      );
